   ...
   // public int SQLcounter = 0;
    ...
    protected void btnCheckSyntax_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            if (checker.errorFound){                                                                // check to see if errorFound was tripped in our instances.
    ...
    protected string ScrubSQLstring(string inputStr)
    {
        ...
        while (inputStr.Contains("  ")) {                                                           // While our input string contains more than one space...
            inputStr = inputStr.Replace("  ", " ");                                                 // then replace those two spaces with one space....repeat as necessary
        }
        while (inputStr.Contains(" ,")) {                                                           // While our input string contains a space before a comma
            inputStr = inputStr.Replace(" ,", ",");                                                 // replace with just a comma.
        }
        while (inputStr.Contains(" ;")) {                                                           // While our input string contains a space before a semicolon
...
//Scrubbed1.Text = scrubbedInput;  // OUTPUT to scrubbed box on page for debugging
//int count = inputStr.Split(' ').Length;     
//string[] strarray = myUtilities.SQLutilities.BuildTokenArray(inputStr, count);