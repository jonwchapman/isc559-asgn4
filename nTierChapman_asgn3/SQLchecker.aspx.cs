...
namespace nTierChapman_asgn3
{
    public partial class SQLchecker : System.Web.UI.Page
    {
        ...
        {
            Key1.Text = "Parsed SQL.";
            string inputStr = TextBox1.Text;                                                            // set the input string equal to the input in Textbox1

            if (inputStr.Length == 0)
            {
                Key1.Text = "No SQL provided.";                                                      // if the user enters no input, then output to error box, letting user know.
            }
            else
            {
             ...
             // pass scrubbed input, and element count to Build Token array, located in the utilities.cs file.
                businesslogic.blSQLapp checker = new businesslogic.blSQLapp();                          // create instance of our SQLutilities class, call it "checker"
              
                string[] tokenArray = checker.BuildTokenArray(inputStr);                                // pass our instance "checker " the input string IOT have it build an array consisting of the components(tokens) of the SQL input.
